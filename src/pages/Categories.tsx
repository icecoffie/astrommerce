...
const Products = () => {
  ...
  type ApiProduct = {
    ...
    total_stock?: number;
    stocks?: Array<{ quantity?: number; stock?: number }>;
  ...
  const fetchProducts = async (type: 'produk' | 'umroh' = 'produk') => {
    ...
    try {
      ...
      const formattedProducts = products.map((p) => {
        // kalau backend kirim array stocks: [{quantity: ...}, ...] → jumlahkan
        const rawStocks = Array.isArray((p as any).stocks) ? (p as any).stocks : [];
        const sumFromArray = rawStocks.reduce(
          (sum: number, s: any) => sum + Number(s?.quantity ?? s?.stock ?? 0),
          0
        );

        // fallback ke total_stock (kalau BE sudah kasih agregat), terakhir 0
        const stock = sumFromArray || Number(p.total_stock ?? 0) || 0;
        ...
        return {
          ...
          merek:
            ...
            (p.supplier_id != null ? supplierMap.get(p.supplier_id) : undefined) ??
            'Tidak Diketahui',
          originPrice: p.price,
          margin: p.margin,
  ...
  const handleShowDetail = (id: number) => {
    ...
    window.location.href = `/admin/ProductDetail/${id}`;
  };
  ...
  return (
    <>
      ...
      <div className="my-4 p-3 rounded-md">
        <CardCategory onCategorySelect={handleCategorySelect} />
        {selectedCategory && (
      ...
      <div className="w-full mt-8">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-5 md:p-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
            {/* Segmented control: Produk / Umroh */}
            <div className="w-full">
              ...
              <div className="inline-flex rounded-xl border border-gray-300 overflow-hidden shadow-sm">
                <button
                  ...
                  className={`px-4 py-2 text-sm ${productType === 'produk'
                    ? 'bg-[#053F8C] text-white'
                ...
                <button
                  ...
                  className={`px-4 py-2 text-sm border-l border-gray-300 ${productType === 'umroh'
                    ? 'bg-[#053F8C] text-white'
                    : 'bg-white text-gray-700 hover:bg-gray-50'
                    }`}
                >
        ...
        <div className="md:hidden px-4 space-y-3 mt-2">
          {paginatedProducts.length === 0 ? (
            ...
            paginatedProducts.map((product) => (
              <div
                key={product.id}
                className="border rounded-xl p-4 shadow-sm flex items-center gap-4"
              >
                <img
                  src={product.image}
                  alt={product.name}
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-gray-500">{product.merek}</div>
                  <div className="mt-1 text-sm text-blue-700 font-bold">
                    {new Intl.NumberFormat('id-ID', {
                      style: 'currency',
                      currency: 'IDR',
                      minimumFractionDigits: 0,
        ...
        {/* Desktop table view */}
        <div className="hidden md:block overflow-x-auto p-4 mt-2 rounded-md">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      name="total_stock"
                      value={editingProduct.total_stock ?? 0}
                      readOnly
                      className="border rounded p-2 w-full"
                    />
                  </div>
